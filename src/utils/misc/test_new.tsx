...
// let _o: number;
// let h_prime: number;
// let h_prime_prime: number;
// /*... actual process for getting the numbers omitted ...*/

// const f = (_o: number, h_prime?: number, h_prime_prime?: number): number => {
// 	const o = +!!(_o > 0) * _o;
// 	// const p = (2 * (+o % 2) - 1) * o;
// 	// const delta = h_prime_prime - h_prime;
// 	// return  h_prime + delta * +(o > 0) + p * +(delta !== 0);
// };
...
const f = (
	_o: number | undefined,
	h_prime: number,
	h_prime_prime: number | undefined
): number => {
	const _h = h_prime_prime ?? 0;
	const delta = _h - h_prime;
	const o = _o ?? 0;
	const p = 2 * (o % 2) - 1;
	return h_prime + delta * +!!o + p * o * +!!delta;
};
// 	/*
//  p * +(delta !== 0) ===0 {
// 	p===0 when, o ==0 => _h ===0; => delta + h

// 	when it has completed a cycle (variable length) _h === h_prime
// 	==> (delta + h)/_h but h_prime may =0

// }

//  */

// 	return _h + delta * o + (p * delta) / delta;
// };
