...
export const getElPointedTop = (...components: ValidComponent[]) => {
	return (
		<div
			style={{
				position: "relative",
	...
	implements IHexagonConstruction
{
	...
	static pointedTopHexPath =
		"M 13.3975 -50 l 0 100 l 86.6025 50 l 86.6025 -50 l -0 -100 l -86.6025 -50 Z";
	static get HexPath() {
		return this.usePointedTop
			? this.pointedTopHexPath
			: this.defaultHexPath;
	}
	...
	private updateGuard(activeHeight: number, height_prime: number) {
		const viewportWidth = window.innerWidth;
		const dampingThreshold = viewportWidth * 0.02;
		if (Math.abs(height_prime - activeHeight) > dampingThreshold) {
		...
		if (this.contentNode) {
			this.contentObserver = new ResizeObserver((entries) => {
				...
				if (entry) {
					...
					this.updateGuard(activeHeight, h_prime);
				}
			});
	...
	render() {
		const {
			args,
			element = (<></>) as ValidComponent,

			usePointedTop = undefined,
			usePointedTopicalAlignment = false,
			...styleProps
		} = this.props;
		...
		let _usePointedTop =
			usePointedTop === undefined ? ctor.usePointedTop : usePointedTop;
		let _element = element;
		if (Array.isArray(_element)) {
			_element = _usePointedTop
				? getElPointedTop(..._element)
				: getElHorizontal(..._element);
		}
			...
			<div style={containerStyle(styleProps)}>
				...
				>
					<div style={elementWrapper}>
						<div style={LeftCutout(_usePointedTop)} />
						<div style={RightCutout(_usePointedTop)} />
						<div
							style={{
								...elementSection,
								paddingTop: usePointedTopicalAlignment
									? `calc(${
